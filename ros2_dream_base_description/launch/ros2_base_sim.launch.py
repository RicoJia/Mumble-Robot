...
def generate_launch_description():
    ...
    no_rviz_arg = DeclareLaunchArgument(
        "no_rviz", default_value="false", description="Flag to disable RViz"
    )

    x_arg = DeclareLaunchArgument(
        "x", default_value="-0.0", description="X position to spawn the robot"
    )

    y_arg = DeclareLaunchArgument(
        "y", default_value="0.0", description="Y position to spawn the robot"
    )

    z_arg = DeclareLaunchArgument(
        "z", default_value="0.05", description="Z position to spawn the robot"
    )
    ...
    set_gazebo_model_path = SetEnvironmentVariable(
        ...
        value=[
            LaunchConfiguration("GAZEBO_MODEL_PATH", default=""),
            ":",
            gazebo_model_path,
        ],
    )
    ...
    gazebo_launch_dir = PathJoinSubstitution([FindPackageShare("gazebo_ros"), "launch"])
