...
KDTree<PointT>::KDTree(const std::shared_ptr<pcl::PointCloud<PointT>> &cloud,
  ...
  if constexpr (dims_ == 3) {
    ...
    std::transform(
        point_idxs.begin(), point_idxs.end(), std::back_inserter(eigen_cloud_),
        [&cloud](const auto &idx) {
          return cloud->points[idx].getVector2fMap().template cast<float>();
        });
  }
  ...
  try {
    ...
    std::for_each(std::execution::par_unseq, index.begin(), index.end(),
                  [this, &query_cloud, &matches, k](size_t idx) {
                    ...
                    search_tree(query_cloud->points[idx], closest_indices, k);
                    // Fill matches vector
...
void KDTree<PointT>::_search_tree(
  ...
  if (knn_results.size() < k || d_split <= skip_tree_dist_coeff_ * d_max) {
