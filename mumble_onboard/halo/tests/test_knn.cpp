...
void evaluate_matches_k_equals_1(
    ...
    const std::vector<halo::NNMatch> &ground_truth_matches, bool print = false,
    halo::CloudPtr first = nullptr, halo::CloudPtr second = nullptr) {
    ...
    for (const auto &test_match : test_matches) {
        if (test_match == ground_truth_matches[test_match.idx_in_this_cloud]) {
            ...
            if (print) {
                size_t test_match_first_cloud_id =
                    test_match.closest_pt_idx_in_other_cloud;
                size_t test_match_second_cloud_id =
                    test_match.idx_in_this_cloud;
                size_t gt_match_first_cloud_id =
                    ground_truth_matches[test_match_second_cloud_id]
                        .closest_pt_idx_in_other_cloud;
                size_t gt_match_second_cloud_id =
                    ground_truth_matches[test_match_second_cloud_id]
                        .idx_in_this_cloud;

                double test_dist = math::get_squared_distance(
                    first->points[test_match_first_cloud_id]
                        .getVector3fMap()
                        .template cast<float>(),
                    second->points[test_match_second_cloud_id]
                        .getVector3fMap()
                        .template cast<float>());

                double gt_dist =
                    // It's slightly faster to use squared distance
                    math::get_squared_distance(
                        first->points[gt_match_first_cloud_id]
                            .getVector3fMap()
                            .template cast<float>(),
                        second->points[gt_match_second_cloud_id]
                            .getVector3fMap()
                            .template cast<float>());

                std::cout << "Test: " << test_match << test_dist << " | gt:"
...
TEST(TestKNN, test_kd_tree) {
    ...
    evaluate_matches_k_equals_1(matches, ground_truth_matches, true, first,
                                second);
